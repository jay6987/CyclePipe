...
#include <future>
#include "../CyclePipe/Timer.h"
#include "../CyclePipe/Pipe.h"
...
namespace UTPipeline
{
	...
	TEST(PipeTest, GetWriteToken)
	{
		...
		// shot end
		{
			auto wt = pipe.GetWriteToken(1, true);
		}
	...
	TEST(PipeTest, AllReadersAndWritersWorkSimultaneously)
	{
		...
		pipe.SetBufferSize(numWriterThreads * nWriteSize +
			numReadThreads * (nReadSize - nOverlapSize) +
			nOverlapSize);
	...
	TEST(PipeTest, ExtendedProperties)
	{
		Pipe<DataType> pipe("pipe_with_properties");
		pipe.SetExtProperty(
			"key",
			std::string("value"));

		auto value0 = pipe.GetExtProperty<std::string>("key");

		ASSERT_EQ("value", value0);

		ASSERT_THROW(
			pipe.GetExtProperty<int>("color"),
			PipeBase::ExtPropertyNotExist
		);
		
		int value = 1;
		pipe.SetExtProperty("value", value);

		value = 0;
		// make sure the value is not referenced
		ASSERT_EQ(pipe.GetExtProperty<int>("value"), 1);

		// could not cast value from int to string
		ASSERT_THROW(
			pipe.GetExtProperty<std::string>("value"),
			std::bad_any_cast
		);
	}
